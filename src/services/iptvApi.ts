...
export class IPTVApi {
     ...
     */
    private buildUrl(action: string, additionalParams: Record<string, string> = {}): string {
        const params = new URLSearchParams({
            username: this.username,
            password: this.password,
            ...additionalParams,
        });
        return `${this.baseUrl}/player_api.php?${params.toString()}&action=${action}`;
    }
    ...
    async getLiveCategories(): Promise<LiveCategory[]> {
        ...
        if (!response.ok) {
            throw new Error(`Failed to fetch live categories: ${response.statusText}`);
        }
     ...
     */
    async getSeriesInfo(seriesId: number): Promise<{ info: SeriesInfo; episodes: Record<string, Episode[]> }> {
